...
import { Expense, ExpenseCategory, AIExtractedExpense } from '../types';
import { extractExpenseFromDocument } from '../services/geminiService';
import { UploadIcon, SparklesIcon } from './Icons';

interface ExpenseFromDocumentProps {
  onDataExtracted: (data: Partial<Expense>) => void;
  categories: ExpenseCategory[];
}

const ExpenseFromDocument: React.FC<ExpenseFromDocumentProps> = ({ onDataExtracted, categories }) => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    setError(null);
    try {
      const extractedData: AIExtractedExpense = await extractExpenseFromDocument(file, categories);
      onDataExtracted({
        ...extractedData,
        ...
        if (err instanceof Error) {
            ...
            setError("Ocurrió un error desconocido durante la extracción.");
        }
    } finally {
      setIsLoading(false);
    }
  }, [onDataExtracted, categories]);
    ...
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in">
      <header className="mb-6">
        <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center">
          ...
          Crear Gasto desde Documento
        </h1>
        <p className="mt-2 text-lg text-gray-600 dark:text-gray-300">
          Sube una foto de un recibo o factura de compra y la IA registrará el gasto por ti.
        </p>
      </header>
          ...
          <div className="flex flex-col items-center justify-center h-48">
            {isLoading ? (
              <>
                ...
                <p className="mt-4 text-lg font-semibold text-gray-700 dark:text-gray-200">Analizando el recibo...</p>
              ...
              <>
                ...
                <p className="mt-4 text-lg font-semibold text-gray-700 dark:text-gray-200">
                  {isDragActive ? "¡Suelta el archivo aquí!" : "Arrastra y suelta tu recibo"}
                </p>
...
export default ExpenseFromDocument;
